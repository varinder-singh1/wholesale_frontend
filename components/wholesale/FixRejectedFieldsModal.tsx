...
const FixRejectedFieldsModal: React.FC<FixRejectedFieldsModalProps> = ({
  ...
  const getRejectedFields = () =>
    Object.entries(formData).filter(([_, field]) => field.status === WHOLESALE_REQUEST_STATUS.rejected);
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white w-full max-w-2xl rounded-lg p-6">
        ...
        <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
          {getRejectedFields().map(([key, field]) => (
            <div key={key}>
              <label className="block font-medium text-sm mb-1">{labelMap[key] || key}</label>
              {key === 'shop_photo' ? (
                <UploadSingleFile
                  name="shop_photo"
        ...
        <div className="flex justify-end mt-6 gap-3">
          <button onClick={() => setShow(false)} className="px-4 py-2 border rounded hover:bg-gray-100">
            Cancel
          </button>
